...
public class AppsForYourDomainClient {
	...
	public AppsForYourDomainClient(String consumerKey, String consumerSecret,
			String domain, String extra) throws OAuthException,
			AuthenticationException {
		this.domain = domain;
		userService = new UserService("ykko-test");
		...
		userService.setOAuthCredentials(oauthParameters,
				new OAuthHmacSha1Signer());
		userService.setReadTimeout(20000);
		userService.setConnectTimeout(20000);

		groupService = new AppsGroupsService(domain, "ykko-test");
		String newUrlEscopo = "http://apps-apis.google.com/a/feeds/group/#readonly";
		oauthParameters.setScope(newUrlEscopo);
		groupService.setOAuthCredentials(oauthParameters,
				new OAuthHmacSha1Signer());
	...
	public void run() throws Exception {
		String randomFactor = Integer.toString(1000 + (new SecureRandom())
				.nextInt(9000));
		...
		UserEntry createdUserEntry = createUser(username, givenName,
				familyName, password);
		...
		NicknameEntry createdNicknameEntry0 = createNickname(username,
				nickname0);
		...
		Iterator<NicknameEntry> nicknameIterator = retrievedNicknameFeed
				.getEntries().iterator();
		...
		LOGGER.log(Level.INFO, "User '" + username
				+ "' has the following nicknames: {" + nicknames.toString()
				+ "}.");
		...
		Iterator<EmailListEntry> emailListIterator = retrievedEmailListFeed
				.getEntries().iterator();
		while (emailListIterator.hasNext()) {
			emailLists
					.append(emailListIterator.next().getEmailList().getName());
			if (emailListIterator.hasNext()) {
				emailLists.append(", ");
			}
		}
		LOGGER.log(Level.INFO, "User '" + username
				+ "' is in the following emailLists: {" + emailLists.toString()
				+ "}.");
		// */
		LOGGER.log(Level.INFO, "Creating users for groups sample run");
		createUser(memberUserName, memberFirstName, memberLastName,
				memberPassword);
		...
		groupsEntry = groupService.createGroup(testGroupId, testGroupName,
				testGroupDescription, "");
		LOGGER.log(Level.INFO, "Group created with following properties:\n"
				+ groupsEntry.getAllProperties());

		groupsEntry = groupService
				.addMemberToGroup(testGroupId, memberUserName);
		...
		groupsEntry = groupService.updateGroup(testGroupId, testGroupName,
		...
		LOGGER.log(Level.INFO,
				testGroupId + " has these members: " + members.toString());
		...
		LOGGER.log(Level.INFO,
				testGroupName + " has these owners: " + owners.toString());
		...
		LOGGER.log(Level.INFO, memberUserName
				+ " is subscribed to these groups:\n" + groups.toString());
		 ...
		 * testGroupId);
		 */
		LOGGER.log(Level.INFO, "Removing " + ownerUserName
				+ " as owner from group " + testGroupId);
	 ...
	 * SimpleCommandLineParser parser = new SimpleCommandLineParser(arg); String
	 * adminEmail = parser.getValue("admin_email", "email", "e"); String
	 * adminPassword = parser.getValue("admin_password", "pass", "p"); String
	 * domain = parser.getValue("domain", "domain", "d");
	 * 
	 * boolean help = parser.containsKey("help", "h"); if (help || (adminEmail
	 * == null) || (adminPassword == null) || (domain == null)) { usage();
	 * System.exit(1); }
	 * 
	 * AppsForYourDomainClient client = new AppsForYourDomainClient( adminEmail,
	 * adminPassword, domain); client.run(); }
	 */
	...
	private static void usage() {
		System.out
				.println("Usage: java AppsForYourDomainClient"
